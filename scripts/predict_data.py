...
def predict_future_kits(df, target_date=None):
    ...
    # Ensure 'dataDate' is in datetime format and set as index
...
def predict_high_risk(df, target_date=None):
    """
    Predicts the total number of high-risk pregnancies for a target month.
    """
    if df is None or df.empty:
        return None, None
        
    # Correct order: set index before resampling
    df['dataDate'] = pd.to_datetime(df['dataDate'])
    df.set_index('dataDate', inplace=True)
